  ...
  <div class="file_container">
      <label>
        ...
        <i @click='choose1' class="imgSize" element-loading-text="图片上传中" title='点击上传' v-loading.fullscreen.lock="fullscreenLoading" v-if="noup && urls.url !== ''"><img :src="urls.url" class="avatar" style="cursor:pointer;width:750px;"></i>
        ...
        <i v-if="urls.url === ''  && noup" title='点击上传' v-loading.fullscreen.lock="fullscreenLoading" class="imgSize" element-loading-text="图片上传中" @click='choose1'>
          <img src="../../../assets/imgs/add-stu.png" class="avatar" style="cursor:pointer;width:200px;">
        </i>
  ...
  methods: {
    ...
    successCallBacks(result) {
      ...
      // 打印
        ...
        console.log('errors2')
        if (img.width !== 750 || img.height !== 352) {
          that.$message.error('请上传750*352px的图片')
          that.fullscreenLoading = false
          return false
        } else {
          that.imgUrl = result.data.access_url
